        ...
        return (
          <div key={card.title} className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-4 sm:p-6 hover:shadow-md transition-shadow">
            ...
            <div className="space-y-1">
              <p className="text-xxs sm:text-xs text-gray-500 dark:text-gray-400">{t('budget.recommended')}: <span className="font-semibold">{card.recommended}</span></p>
              <p className="text-xxs sm:text-xs text-gray-500 dark:text-gray-400">{t('budget.actual')}: <span className="font-semibold">{card.actual}</span></p>
              <p className="text-xxs sm:text-xs text-gray-500 dark:text-gray-400">{t('budget.difference')}: <span className={`font-semibold ${card.color}`}>{card.difference}</span></p>
            </div>
          </div>
        );
      })}
