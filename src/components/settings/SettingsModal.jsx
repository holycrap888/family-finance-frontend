      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg p-4 sm:p-6 w-full max-w-md max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between mb-4 sm:mb-6">
          <div className="flex items-center min-w-0 flex-1">
            <SettingsIcon className="h-5 w-5 sm:h-6 sm:w-6 text-indigo-600 dark:text-indigo-400 mr-2 flex-shrink-0" />
            <h2 className="text-lg sm:text-xl font-bold text-gray-900 dark:text-white truncate">Budget Settings</h2>
          </div>
          <button
            onClick={onClose}
            className="text-gray-400 hover:text-gray-600 dark:text-gray-300 dark:hover:text-gray-100 transition-colors p-1 flex-shrink-0 ml-2"
            disabled={loading}
          >
        ...
        <div className="mb-4 p-3 bg-blue-50 dark:bg-blue-900/30 rounded-lg text-xs sm:text-sm text-blue-700 dark:text-blue-300">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          {budgetCategories.map(({ key, label, description }) => (
            <div key={key}>
              <div className="flex items-center justify-between mb-1">
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 truncate flex-1">
                  {label}
                </label>
              ...
              <p className="text-xxs sm:text-xs text-gray-500 dark:text-gray-400 mb-2">{description}</p>
              <div className="relative">
                <input
                  type="number"
                  min="0"
                  max="100"
                  className="w-full px-3 py-2 pr-8 border border-gray-300 dark:border-gray-600 rounded-md focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-colors bg-white dark:bg-gray-700 text-gray-900 dark:text-white text-sm"
          ...
          <div className="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-3 pt-4">
            <button
              ...
              onClick={onClose}
              disabled={loading}
              className="w-full sm:flex-1 px-4 py-2 text-gray-700 dark:text-gray-300 bg-gray-100 dark:bg-gray-700 rounded-md hover:bg-gray-200 dark:hover:bg-gray-600 disabled:opacity-50 transition-colors text-sm sm:text-base"
            >
              Cancel
            </button>
            <button
              ...
              className="w-full sm:flex-1 px-4 py-2 bg-indigo-600 dark:bg-indigo-500 text-white rounded-md hover:bg-indigo-700 dark:hover:bg-indigo-600 disabled:opacity-50 transition-colors flex items-center justify-center text-sm sm:text-base"
            >
              {loading ? (
                <>
